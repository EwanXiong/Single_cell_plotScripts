...
parser$add_argument("--coord_flip", action='store_true', help = "whether flip the x and y")
parser$add_argument("--height", help = "pdf height")
parser$add_argument("--width",help = "pdf width")
...
parserMarker <- function(seurat_object, preanno=FALSE, marker=NULL){
...
StackVlnPlot <- function(obj, feature, pt.size = 0, plot.margin = unit(c(0,0,0,0),"cm"), coord_flip = FALSE,...){
    plot_list <- purrr::map(feature,function(x) modify_vlnplot(obj, feature = x, coord_flip = coord_flip,...))
    if(!coord_flip){
        ...
        p <- patchwork::wrap_plots(plotlist = plot_list,ncol = length(feature))
    }
    return(p)
}
...
main <- function(){
    ...
    feature <- parserMarker(seurat_object, args$preanno, marker)

    p <- StackVlnPlot(seurat_object, feature = feature, pt.size = 0, col = color, coord_flip = args$coord_flip)
    if(args$coord_flip){
        w <- 10
        h <- 7
    }else{
    ...
    ggsave(p, filename = gsub("pdf$", "png", pdf), width=w, height=h, bg="white")
